...
{
    include '../../../../database/database.php';
    $sql = "SELECT product_tags.id, product_tags.name
        FROM product_tags
        INNER JOIN link_product_tag ON link_product_tag.tag_id = product_tags.id
        INNER JOIN products ON link_product_tag.product_id = products.id
...
{
    include '../../../../database/database.php';
    $sql = "SELECT news_tags.id, news_tags.name
            FROM news_tags
            INNER JOIN link_news_tag ON link_news_tag.tag_id = news_tags.id
            INNER JOIN news ON link_news_tag.news_id = news.id
...
{
    include '../../../../database/database.php';
    $sql = "SELECT link_role_permission.*
        FROM link_role_permission
        INNER JOIN permissions ON permissions.id = link_role_permission.permission_id
        INNER JOIN roles ON roles.id = link_role_permission.role_id
        INNER JOIN users ON users.role_id = roles.id
        WHERE users.id = '$user_id' AND permissions.code = '$per_code'";
    $result = $conn->query($sql);
    if (!$result) {
        return false;
    }
...
function uploadImages($folder, $location){
    ...
    $file_store = "../../../../public/admin/assets/images/" . $folder . "/" . $file_name;
    $uploadOk = 1;
